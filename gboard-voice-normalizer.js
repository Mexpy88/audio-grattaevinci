...
(function installWarehouseGboardNormalizer(){
  ...
  const VERSION='2026.08.25-gboard-max2';
  const SEP='[\\s,.;:_\\-/]*';
  const DIGIT_WORDS={
  ...
  function literalArticleRegex(article){
    const body=canonical(article).replace(/^I/,'').toLowerCase();
    const parts=[...body].join(SEP);
    return new RegExp('\\b(?:(?:i|1)'+SEP+')?'+parts+'\\b','gi');
  }
  function charPart(ch){
    if(DIGIT_WORDS[ch])return '(?:'+DIGIT_WORDS[ch].join('|')+')';
    const arr=LETTER_WORDS[ch]||[String(ch).toLowerCase()];return '(?:'+arr.join('|')+')';
  }
  function spokenArticleRegex(article){
  ...
  function normalizeKnown(raw){
    ...
    for(const article of knownArticles()){
      try{text=text.replace(literalArticleRegex(article),article)}catch{}
      try{text=text.replace(spokenArticleRegex(article),article)}catch{}
    }
    return text;
  }
