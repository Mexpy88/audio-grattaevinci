/* Robustezza UI generale.
   REFRACTOR 2026-08-28: questo modulo NON possiede più il flusso Master Excel.
   L'importazione è gestita da WarehouseMasterControllerV1 con un solo handler autorevole. */
(function installWarehouseUiHardening(){
  'use strict';
  if(window.WarehouseUIHealth?.version==='2026.08.28-ui-hardening-clean1')return;
  const VERSION='2026.08.28-ui-hardening-clean1';
  ...
  function auditButtons(root=document){
    ...
    if(document.documentElement)document.documentElement.dataset.uiHealth=lastReport.ok?'ok':'error';
  ...
  /* Compatibility API: historical modules call hardenMasterConfirm().
     It deliberately does not wrap or replace importMappedMaster anymore. */
  function hardenMasterConfirm(){
    const btn=document.querySelector?.('#masterDialog .btn.success');
    if(!btn)return false;
    btn.type='button';
    btn.dataset.hardImportBound='controller';
    return true;
  }
  ...
  function install(){
    installErrorBoundary();hardenMasterConfirm();auditButtons(document);
    if(!window.__warehouseUiObserver){
      window.__warehouseUiObserver=new MutationObserver(records=>{
        if(records.some(r=>r.addedNodes?.length)){hardenMasterConfirm();auditButtons(document)}
      });
      window.__warehouseUiObserver.observe(document.body,{childList:true,subtree:true});
    }
    return true;
  }
  window.WarehouseUIHealth={version:VERSION,run:()=>auditButtons(document),getReport:()=>lastReport,hardenMasterConfirm,install};
  install();
})();
