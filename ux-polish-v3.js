...
(function installWarehouseUxPolishV3(){
  ...
  function loadRoleDashboard(){
    ...
    /* Role Dashboard V1 used to rebuild #rdDashboardV1 from scratch every 1.5 seconds.
       That periodic render conflicts with composable modules such as Entrata Merci and also wastes CPU.
       Suppress only that legacy interval while the script bootstraps; all explicit/event-driven renders remain. */
    const realSetInterval=window.setInterval;let restored=false;
    const restore=()=>{if(restored)return;restored=true;window.setInterval=realSetInterval};
    window.setInterval=function(fn,delay,...args){
      if(Number(delay)===1500&&String(fn).includes('renderDashboard'))return 0;
      return realSetInterval.call(window,fn,delay,...args);
    };

    const s=document.createElement('script');s.id='roleDashboardV1Js';s.src='role-dashboard-v1.js?v=20260828-role-dashboard-event1';s.async=false;
    s.onload=()=>{restore();window.WarehouseRoleDashboardV1?.install?.();loadRolePatch()};
    s.onerror=()=>{restore();console.error('Impossibile caricare role-dashboard-v1.js')};
