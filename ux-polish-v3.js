/* Final UX polish for REMOTO V1 managerial interface.
   ...
   - bootstraps Role Dashboard V1 + Premium Dashboard V2 + mobile request confirmation + registry repair + Master import hardening
*/
(function installWarehouseUxPolishV3(){
  'use strict';
  if(window.WarehouseUxPolishV3)return;

  const VERSION='2026.08.27-ux-polish-v3-premium2-floating-confirm1-registryfix1-masterfix1';
  ...
  function loadMasterImportFix(){
    if(window.WarehouseMasterImportFixV1){window.WarehouseMasterImportFixV1.install?.();return true}
    if(document.getElementById('masterImportFixV1Js'))return true;
    const m=document.createElement('script');m.id='masterImportFixV1Js';m.src='master-import-fix-v1.js?v=20260827-masterfix1';m.async=false;m.onload=()=>window.WarehouseMasterImportFixV1?.install?.();m.onerror=()=>console.error('Impossibile caricare master-import-fix-v1.js');document.body.appendChild(m);return true;
  }
  function loadRegistryFix(){
    if(window.WarehouseRegistryMovementsFixV1){window.WarehouseRegistryMovementsFixV1.install?.();loadMasterImportFix();return true}
    if(document.getElementById('registryMovementsFixV1Js')){loadMasterImportFix();return true}
    const r=document.createElement('script');r.id='registryMovementsFixV1Js';r.src='registry-movements-fix-v1.js?v=20260827-registryfix1';r.async=false;r.onload=()=>{window.WarehouseRegistryMovementsFixV1?.install?.();loadMasterImportFix()};r.onerror=()=>console.error('Impossibile caricare registry-movements-fix-v1.js');document.body.appendChild(r);return true;
  }
  ...
  window.WarehouseUxPolishV3={version:VERSION,install,hideRegistryMaster,loadRoleDashboard,loadRolePatch,loadPremiumDashboard,loadPremiumFix,loadFloatingConfirm,loadRegistryFix,loadMasterImportFix};
  install();
})();
