...
const api=context.WarehouseMobileSearchV3;if(!api)throw new Error('API not exposed');
...
const p=api.parseSearch('I00215-S');const matched=rows.filter(r=>api.rowMatchesSearch(r,p));if(matched.length!==1||matched[0].size!=='S'||matched[0].article_base!=='I00215')throw new Error('Exact article+size filtering failed');
if(!api.hasPosition('13','')||!api.hasPosition('','A POSTO1')||!api.hasPosition('13','A POSTO1')||api.hasPosition('',''))throw new Error('Flexible position logic failed');
...
console.log('Mobile Search V3 runtime OK: exact size parsing/filtering and flexible positions verified.');
