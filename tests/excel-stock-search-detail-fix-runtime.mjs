...
if(api.lookupKeyFormula(15)!=='TRIM($B$3)&"|"&TRIM($B$4)&"|1')throw new Error('First visible row must use scalar helper key #1');
if(api.lookupKeyFormula(16)!=='TRIM($B$3)&"|"&TRIM($B$4)&"|2')throw new Error('Second visible row must use scalar helper key #2');
...
for(let col=0;col<7;col++){
  ...
  if(!f.includes("MATCH(TRIM($B$3)&\"|\"&TRIM($B$4)&\"|1,'GIACENZE_RICERCA_DATI'!$H$2:$H$500,0)"))throw new Error(`Column ${letter} does not use scalar MATCH helper key`);
...
const second=api.detailFormula(1,16,500);if(!second.includes('"|2'))throw new Error('Second detail row must request helper key #2');
const twentieth=api.detailFormula(5,34,500);if(!twentieth.includes('"|20'))throw new Error('Row 34 must request helper key #20');
...
console.log('Excel search detail fix OK: each position uses scalar INDEX+MATCH against deterministic helper keys, with no array or implicit-intersection dependency.');
