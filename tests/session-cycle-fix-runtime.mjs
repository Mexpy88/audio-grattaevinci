...
const oldMovements=Array.from({length:44},(_,i)=>({id:'OLD'+i,registered_at:before,operation_at:before}));
const shown=[];
const context={
  window:null,
  console,
  ...
  show:id=>{shown.push(id);return id},
  db:{
    ...
    audits:Array.from({length:44},(_,i)=>({id:'A'+i,action:'CREATE',at:before}))
  }
};
...
api.wrapShow();
context.show('registryScreen');
context.show('home');
if(shown.join(',')!=='registryScreen,home')throw new Error('Home navigation wrapper changed original show behavior');
