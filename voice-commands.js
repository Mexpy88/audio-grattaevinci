...
(function installWarehouseVoiceCommands(){
  ...
  function parseCommand(raw,hint='AUTO'){
    const s=clean(raw),action=actionFrom(s,hint),parsedState=parseState(s),sourceLocation=parseSourceLocation(s),sourcePallet=parseSourcePallet(s),explicitTargetLocation=parseTargetLocation(s),explicitTargetPallet=parseTargetPallet(s),genericLocation=parseLocation(s),genericPallet=parsePallet(s);
    const cmd={raw:String(raw||''),action,article:parseArticle(s),size:parseSize(s),quantity:parseQuantity(s),state:action==='MODIFICA'?'':parsedState,targetState:action==='MODIFICA'?parsedState:'',destination:parseDestination(s),location:'',pallet:'',sourceLocation,sourcePallet,targetLocation:'',targetPallet:''};
    if(action==='MODIFICA'){
  ...
  function ensureMasterAccess(){
    try{if(window.LocalMaster?.requireMaster)return !!window.LocalMaster.requireMaster()}catch{}
    return true;
  }
  ...
  function executeSearch(cmd){
    if(!ensureMasterAccess())return false;if(!cmd.article&&!cmd.size){notify('Per la ricerca pronuncia almeno il codice articolo o la taglia.','error');return false}
  ...
  function setDestination(value){
    const select=$id('destination');if(!select||!value)return;const target=clean(value);const opt=[...select.options].find(o=>clean(o.value)===target||clean(o.textContent)===target);if(opt)select.value=opt.value;
  }
  function resolveDischargeState(cmd){
    if(cmd.state)return cmd.state;const probe={...cmd,state:''},rows=stockRows().filter(r=>matches(r,probe));const states=[...new Set(rows.map(r=>norm(r.state)).filter(Boolean))];return states.length===1?states[0]:'';
  }
  function setResultsVoiceMode(active){
    if(typeof document==='undefined')return;const back=document.querySelector('#results .back');if(back)back.setAttribute('onclick',active?"show('operation')":RESULTS_BACK_DEFAULT);const box=$id('voiceReviewSummary');if(box&&!active)box.remove();
  }
  ...
  function executeOperation(cmd){
    if(!ensureMasterAccess())return false;if(!cmd.article){notify('Non ho riconosciuto il codice articolo.','error');return false}if(!(Number(cmd.quantity)>0)){notify('Non ho riconosciuto una quantità valida.','error');return false}if(!cmd.location||!cmd.pallet){notify('Pronuncia anche Fila/Scaffale e Bancale.','error');return false}
    let state=cmd.state;if(cmd.action==='CARICA'&&!state)state='NUOVO';if(cmd.action==='SCARICA'&&!state)state=resolveDischargeState(cmd);if(!state){notify('Per lo scarico ci sono più stati possibili: pronuncia NUOVO, SCARICATO oppure USATO.','error');return false}
    if(typeof openOperation!=='function')return false;openOperation(cmd.action);if($id('filaScaffale'))$id('filaScaffale').value=cmd.location;if($id('bancale'))$id('bancale').value=cmd.pallet;if(cmd.action==='SCARICA'&&cmd.destination)setDestination(cmd.destination);
    try{importedPhotos=[{photo_index:1,general_note:'Comando vocale',groups:[{article_base:cmd.article,description:'',confidence:1,variants:[{size:cmd.size||'',quantity:Number(cmd.quantity),state,confidence:1,note:''}]}]}];renderResults();show('results');setResultsVoiceMode(true);voiceReviewSummary(cmd,state)}catch(e){notify('Non sono riuscito a preparare l’operazione: '+(e.message||e),'error');return false}
    notify('Operazione compilata. Controlla i dati e premi CONFERMA TUTTO.','good');return true;
  }
  function candidateRowsForModify(cmd){
    let rows=stockRows();if(cmd.article)rows=rows.filter(r=>normalizedArticle(r.article_base)===normalizedArticle(cmd.article));if(cmd.size)rows=rows.filter(r=>norm(r.size)===norm(cmd.size));if(cmd.sourceLocation)rows=rows.filter(r=>norm(typeof locationOf==='function'?locationOf(r):r.fila_scaffale)===norm(cmd.sourceLocation));if(cmd.sourcePallet)rows=rows.filter(r=>norm(r.bancale)===norm(cmd.sourcePallet));return rows;
  }
  function executeModify(cmd){
    if(!ensureMasterAccess())return false;if(typeof openStockEdit!=='function'||typeof loadStockPallet!=='function'){notify('Modulo MODIFICA non disponibile.','error');return false}
    if(!cmd.article){
    ...
    const candidates=candidateRowsForModify(cmd);if(!candidates.length){notify('Non trovo una giacenza corrispondente da modificare.','error');return false}if(candidates.length>1){notify('Ho trovato più giacenze corrispondenti. Specifica la taglia e, se necessario, la fila o il bancale di origine.','warn');return false}
    const source=candidates[0],sourceLoc=norm(typeof locationOf==='function'?locationOf(source):source.fila_scaffale),sourcePal=norm(source.bancale);openStockEdit();$id('stockEditLocation').value=sourceLoc;$id('stockEditPallet').value=sourcePal;loadStockPallet();
    ...
    if(cmd.quantity!==null&&cmd.quantity!==undefined)draft.quantity=Math.max(0,Number(cmd.quantity)||0);if(cmd.targetState)draft.state=cmd.targetState;if(cmd.targetLocation)draft.fila_scaffale=cmd.targetLocation;if(cmd.targetPallet)draft.bancale=cmd.targetPallet;
    try{renderStockEditRows();if(typeof setStatus==='function')setStatus('stockEditSearchStatus','Comando vocale applicato. Controlla attentamente i valori e poi premi SALVA MODIFICHE.','good')}catch{}
  ...
  function makeContextBlock(id,label,hint){const block=document.createElement('div');block.id=id;block.className='voiceContextBlock';block.innerHTML=`<button type="button" class="voiceBtn"><span class="voiceMicPulse">🎙</span> ${html(label)}</button><div class="voiceStatus" aria-live="polite"></div>`;block.querySelector('button').addEventListener('click',()=>start(hint));return block}
  function wrapOpenOperation(){const base=window.openOperation;if(typeof base!=='function'||base.__voiceReviewWrapped)return;const wrapped=function(){setResultsVoiceMode(false);return base.apply(this,arguments)};wrapped.__voiceReviewWrapped=true;window.openOperation=wrapped}
  function installUi(){
    if(typeof document==='undefined')return false;wrapOpenOperation();
    ...
    const search=$id('searchScreen');if(search&&!$id('voiceSearchBlock'))search.querySelector('.card')?.appendChild(makeContextBlock('voiceSearchBlock','CERCA CON LA VOCE','CERCA'));
    const op=$id('operation');if(op&&!$id('voiceOperationBlock'))op.querySelector('h1')?.insertAdjacentElement('afterend',makeContextBlock('voiceOperationBlock','DETTA OPERAZIONE','AUTO'));
    const edit=$id('stockEditScreen');if(edit&&!$id('voiceModifyBlock'))edit.querySelector('.card')?.appendChild(makeContextBlock('voiceModifyBlock','MODIFICA CON LA VOCE','MODIFICA'));
    return true;
  }
