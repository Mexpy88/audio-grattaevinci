...
assert.match(direct,/<title>Magazzino NOVA · Teramo · V1[789]<\/title>/);
...
const rows=[{location:'5',pallet:'157'},{location:'52',pallet:'5'},{location:'15',pallet:'DISMESSI 52'},{location:'5',pallet:'DISMESSI 52'},{location:'',pallet:'5'}];
...
const script=(direct.match(/<script>([\s\S]*?)<\/script>/)||[])[1];assert.ok(script,'application script missing');new vm.Script(script,{filename:'nova-position-regression.js'});console.log('NOVA exact independent position engine + UX regression: OK');
