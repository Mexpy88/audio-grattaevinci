...
(function installRectificationFlags(){
  ...
  const VERSION='2026.08.25-rect-flags2';
  ...
  function flagKey(row){if(!row)return '';return [article(row.article_base),norm(row.size),locOf(row),norm(row.bancale)].join('|')}
  function ensureStore(){if(typeof db==='undefined')return {};if(!db[STORE]||typeof db[STORE]!=='object'||Array.isArray(db[STORE]))db[STORE]={};return db[STORE]}
  ...
  function setFlag(row,flagged){const k=flagKey(row);if(!k)return false;const store=ensureStore();if(flagged){store[k]={flagged:true,at:nowIso(),operator:operator(),article_base:article(row.article_base),size:norm(row.size),fila_scaffale:locOf(row),bancale:norm(row.bancale)}}else delete store[k];save();return !!flagged}
  function moveFlag(oldRow,newRow){
    ...
    let oldStillUsed=false;try{oldStillUsed=(stockEditRowsDraft||[]).some(r=>!r.deleted&&r!==newRow&&flagKey(r)===oldKey)}catch{}
    if(!oldStillUsed)delete store[oldKey];save();return true;
  }
  function uniqueDraftStats(){const keys=new Set(),flagged=new Set();try{for(const r of (stockEditRowsDraft||[])){if(r.deleted)continue;const k=flagKey(r);if(!k)continue;keys.add(k);if(ensureStore()[k])flagged.add(k)}}catch{}return {total:keys.size,flagged:flagged.size}}
  function syncDraftFlags(){try{for(const r of (stockEditRowsDraft||[]))r.rectification_flagged=isFlagged(r)}catch{}}
  ...
  function decorateRows(){
    ...
    nodes.forEach((node,i)=>{
      ...
      const head=node.querySelector('.stockEditHead');if(!head)return;
      node.querySelectorAll('.rectFlagMeta').forEach(x=>x.remove());
      head.querySelectorAll('.rectFlagWrap').forEach(x=>x.remove());
    ...
    const summary=document.getElementById('stockEditSummary'),st=uniqueDraftStats();if(summary){summary.querySelectorAll('.rectFlagSummary').forEach(x=>x.remove());const x=document.createElement('span');x.className='rectFlagSummary';x.textContent=` · MODIFICATI ${st.flagged}/${st.total}`;summary.appendChild(x)}
  }

  function installWrappers(){
    if(typeof window.stockEditBuildDraft==='function'&&!window.stockEditBuildDraft.__rectFlags){baseBuild=window.stockEditBuildDraft;const wrapped=function(...args){const out=baseBuild.apply(this,args);syncDraftFlags();return out};wrapped.__rectFlags=true;window.stockEditBuildDraft=wrapped}
    if(typeof window.renderStockEditRows==='function'&&!window.renderStockEditRows.__rectFlags){baseRender=window.renderStockEditRows;const wrapped=function(...args){const out=baseRender.apply(this,args);decorateRows();return out};wrapped.__rectFlags=true;window.renderStockEditRows=wrapped}
    if(typeof window.editStockDraft==='function'&&!window.editStockDraft.__rectFlags){
      baseEdit=window.editStockDraft;const wrapped=function(id,key,value){let d=null,oldSnapshot=null,wasFlagged=false;try{d=(stockEditRowsDraft||[]).find(r=>r.edit_id===id);if(d){oldSnapshot={...d};wasFlagged=isFlagged(d)}}catch{}
        const out=baseEdit.apply(this,arguments);if(d&&wasFlagged&&['article_base','size','fila_scaffale','bancale'].includes(key))moveFlag(oldSnapshot,d);if(d)d.rectification_flagged=isFlagged(d);return out};wrapped.__rectFlags=true;window.editStockDraft=wrapped;
    }
  }
