...
5. **Una sola responsabilità per motore.** `StockService`, `ReceivingService`, `RequestService`, `MovementService`, `ExcelService` e `Router` non vengono sostituiti a runtime.
6. **Event-driven.** Nessun polling UI. I render avvengono soltanto a seguito di navigazione o eventi dati espliciti.
7. **REMOTO è read-only per NOVA.** L'eventuale migrazione copia i dati; non scrive mai `so_magazzino_v2`.
