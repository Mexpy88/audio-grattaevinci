...
(function installStockControlAssistedUxV4(){
  ...
  const VERSION='2026.08.26-stock-control-assisted-ux4';
  ...
  function injectCss(){
    ...
    const s=document.createElement('style');s.id='stockControlAssistedUxV4Css';s.textContent=`
      ...
      .scaFloatAdd{background:#17314d!important}.scaFloatAdd::first-letter{font-weight:950}
      .scaQuickDialog #scaQuickTitle{font-size:21px}
  ...
  function stateFromVariant(v){const sel=v.querySelector('select');if(sel)return norm(sel.value)||'NON_CHIARO';const raw=v.querySelector('.scaVariantTitle')?.textContent||'';const bits=raw.split('·').map(norm).filter(Boolean);return bits.at(-1)||'NON_CHIARO'}
  function sizeFromVariant(v){const input=v.querySelector('input.field:not([type="number"])');if(input&&v.closest('#stockEditScreen'))return norm(input.value)||'SENZA TAGLIA';const raw=v.querySelector('.scaVariantTitle')?.textContent||'';return norm(raw.split('·')[0])||'SENZA TAGLIA'}
  ...
  function applySearch(){const q=norm($('scaArticleSearchV4')?.value);document.querySelectorAll('#scaCountRows>.scaArticleGroup').forEach(g=>{const show=!q||articleCodeOf(g).includes(q);g.style.display=show?'':'none'})}
  ...
  function updateProgress(){
    const body=$('scaCountBody');if(!body||body.classList.contains('hidden'))return;ensureTools();const s=summaryFromDom(),pct=s.total?Math.round(s.verified*100/s.total):0;safeText($('scaProgressTextV4'),`${s.verified}/${s.total} verificate · ${s.total-s.verified} da controllare`);safeText($('scaProgressPositionV4'),`${pct}%`);const fill=$('scaProgressFillV4');if(fill&&fill.style.width!==pct+'%')fill.style.width=pct+'%';
    ...
    const loc=norm($('scaCountLoc')?.value),pal=norm($('scaCountPal')?.value);safeText($('scaProgressPositionV4'),`${loc?`Fila ${loc}`:''}${loc&&pal?' · ':''}${pal?`Bancale ${pal}`:''}${(loc||pal)?` · ${pct}%`:`${pct}%`}`);applySearch();
  }
  ...
  function decorate(){
    scheduled=false;injectCss();renameAddActions();ensureHelp();ensureTools();document.querySelectorAll('.scaArticleGroup').forEach(decorateArticleGroup);document.querySelectorAll('.scaVariant').forEach(decorateVariant);updateProgress();
  }
  ...
  function install(){
    ...
    document.addEventListener('input',e=>{if(e.target?.closest?.('.scaArticleGroup'))schedule()},true);document.addEventListener('change',e=>{if(e.target?.closest?.('.scaArticleGroup'))schedule()},true);
    return true;
  }
